...
import sys, glob, csv, pytz, shutil, os  # добавив os
...
def get_email(prsn=None):
    ...
    if c == 0:
        email = (
            prsn.first_name
            + get_formatted_datetime("%Y", prsn.birthdate, "%d %b, %Y")
            + dmn
            + "."
            + ext
        )
        ...
        email = (
            prsn.last_name
            + get_formatted_datetime("%d", prsn.birthdate, "%d %b, %Y")
            + dmn
            + "."
            + ext
        )
...
def get_alphabetic_profile_img(char, filePath, imgName, charColor=None, bgColor=None):
    chars = (
        "qwertyuioplkjhgfdsazxcvbnmQAZXSWEDCVFRTGBNHYUJMKLIOP0123456789 ,.+=-_()[]{}"
    )
    ...
    if all((c in chars) for c in imgName):
        if access(dirname(filePath), W_OK):
            if gender == None:
                ...
                orig_file = choice(
                    glob.glob(dir_name + "\\images\\people\\female_*.jpg")
                )
...
def get_birthdate(startAge=None, endAge=None, _format="%d %b, %Y"):
    ...
    if startAge != None and endAge != None:  # If both are given in arg
        if startAge >= endAge:
            ...
            startRange = datetime(
                datetime.now().year - startAge, 12, 31, 23, 59, 59, 0, pytz.UTC
            )
            endRange = datetime(
                datetime.now().year - endAge, 1, 1, 0, 0, 0, 0, pytz.UTC
            )
    elif startAge != None or endAge != None:  # If anyone is given in arg
        ageYear = startAge if startAge != None else endAge
        startRange = datetime(
            datetime.now().year - ageYear, 12, 31, 23, 59, 59, 0, pytz.UTC
        )
...
def get_address():  # переписав цю функцію
    import os

    full_addr = []
    addrParam = ["street", "landmark", "area", "city", "state", "country", "pincode"]

    # Правильний шлях до data.csv
